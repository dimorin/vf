    ...
    <v-list>
      ...
      >
        <template v-slot:activator>
          <v-list-item-content>
            <v-list-item-title>
              {{item.title}}
              <span>
                <v-btn icon x-small @click="openDialogItem(i)"><v-icon>mdi-pencil</v-icon></v-btn>
                <v-btn icon x-small @click="moveItem(items,i,-1)" v-if="i>0"><v-icon>mdi-chevron-double-up</v-icon></v-btn>
                <v-btn icon x-small @click="moveItem(items,i,1)" v-if="i<items.length-1"><v-icon>mdi-chevron-double-down</v-icon></v-btn>
              </span>
            </v-list-item-title>
          </v-list-item-content>
        </template>
        ...
        >
          <v-list-item-content>
            <v-list-item-title>
              {{ subItem.title }}
              <span>
                <v-btn icon x-small @click="openDialogSubItem(i, j)"><v-icon>mdi-pencil</v-icon></v-btn>
                <v-btn icon x-small @click="moveItem(item.subItems,j,-1)" v-if="j>0"><v-icon>mdi-chevron-double-up</v-icon></v-btn>
                <v-btn icon x-small @click="moveItem(item.subItems,j,1)" v-if="j<item.subItems.length-1"><v-icon>mdi-chevron-double-down</v-icon></v-btn>
              </span>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-list-item @click="openDialogSubItem(i, -1)" >
          ...
          <v-list-item-content>
            <v-list-item-title>2depth 추가하기</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list-group>
      <v-list-item @click="openDialogItem(-1)" >
        ...
        <v-list-item-content>
          <v-list-item-title>1depth 추가하기</v-list-item-title>
        </v-list-item-content>
      </v-list-item>
...
<script>
export default {
  // https://github.com/fkkmemi/vf2/blob/master/src/views/site/menu.vue
  props: {
    items: Array
  },
  data () {
